...
const students = [
    { id: "1", name: "김민준", status: "active", grade: "초3-1" },
    { id: "2", name: "이서연", status: "active", grade: "초3-2" },
    { id: "3", name: "박지후", status: "active", grade: "초4-1" },
    { id: "4", name: "최지우", status: "active", grade: "초4-2" },
    { id: "5", name: "윤준서", status: "active", grade: "초5-1" },
    { id: "6", name: "이지아", status: "active", grade: "초6-1" },
    { id: "7", name: "백다혜", status: "active", grade: "중1-1" },
    { id: "8", name: "임채원", status: "active", grade: "중2-1" },
    { id: "9", name: "조도현", status: "stopped", grade: "중3-1" },
    { id: "10", name: "고서영", status: "active", grade: "초5-2" }
];
...
for (let mOffset = 0; mOffset < 12; mOffset++) {
    ...
    const todayCutoff = new Date(2026, 1, 4); // 2026-02-04 (exclusive)
    ...
    ["math", "science"].forEach(sub => {
        ...
        students.forEach((s, sIdx) => {
            ...
            for (let d = 1; d <= daysInMonth; d++) {
                ...
                const isFutureDate = date >= todayCutoff;

                if (isSystemOff || s.status === "stopped" || isFutureDate) {
...
const content = `
export const MOCK_STUDENTS = ${JSON.stringify(students)};
export const MATH_UNITS = ${JSON.stringify(mathUnits)};
export const SCIENCE_UNITS = ${JSON.stringify(scienceUnits)};
export const MOCK_MONTHLY_SCORES = ${JSON.stringify(monthlyScores)};
export const MOCK_DAILY_DETAILS = ${JSON.stringify(dailyDetails)};
`;

fs.writeFileSync('src/app/admin/learning-history/mockData.ts', content);
