...
import { INITIAL_TASKS, MATH_CURRICULA } from "@/lib/task-center-mock";
...
export default function MathResultPage(props: PageProps) {
  ...
  useEffect(() => {
    ...
    let currentTask = taskList.find((t) => matchStoredIdWithAdminId(t.id, taskId) || matchStoredIdWithAdminId(taskId, t.id));
    if (currentTask) {
  ...
  const handleVerifyExamPrep = () => {
    ...
    router.push(`/content/math-exam-prep?fromTaskResult=true&highlightTypeIds=${typeIdsParam}&gradeSemester=${encodeURIComponent(task?.course || "")}&sourceTaskId=${taskId}&source=task-center`);
  };
    ...
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900 transition-colors duration-200 dark:bg-slate-950 dark:text-slate-100 pb-24">
      ...
      <header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 z-40">
        <div className="flex items-center space-x-2.5 text-sm font-semibold">
          {/* 과목 뱃지 */}
          <span className="inline-flex items-center rounded-md px-2 py-0.5 text-xs font-bold bg-blue-50 text-blue-700 border border-blue-150 dark:bg-blue-950/40 dark:text-blue-300 dark:border-blue-900">
            수학
          </span>
        ...
        <section className="w-full pb-6 border-b border-slate-200 dark:border-slate-800">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            ...
            <div className="flex items-baseline gap-4">
              ...
              <span className="text-5xl font-black text-blue-600 dark:text-blue-450 font-mono tracking-tight">
                {taskResult.score}
            ...
            <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-slate-650 dark:text-slate-350">
              <div className="flex items-center gap-2">
                ...
                <span className="font-extrabold text-emerald-600 dark:text-emerald-400">{taskResult.correctCount} / {task.totalProblems}</span>
              </div>
              <span className="h-3.5 w-px bg-slate-200 dark:bg-slate-800" />
              <div className="flex items-center gap-2">
                <span className="font-bold text-slate-400">오답</span>
                <span className="font-extrabold text-rose-600 dark:text-rose-450">{taskResult.incorrectCount}문항</span>
              </div>
              <span className="h-3.5 w-px bg-slate-200 dark:bg-slate-800" />
              <div className="flex items-center gap-2">
        ...
        <section className="w-full">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 text-xs text-slate-500 dark:text-slate-450 font-bold uppercase tracking-wider">
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-20 bg-white/95 dark:bg-slate-900/95 border-t border-slate-200 dark:border-slate-800 backdrop-blur-md z-45 flex items-center justify-end px-8 shadow-inner">
        <div className="flex items-center space-x-4">
          ...
          <button
            onClick={() => router.push(`/content/math-task-center/${taskId}/explanation${isPreview ? "?preview=true" : ""}`)}
            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 dark:bg-blue-600 dark:hover:bg-blue-500 text-white text-sm font-extrabold rounded-xl shadow-md hover:shadow-lg transition-all active:scale-95 flex items-center gap-1.5 group cursor-pointer"
          >
            <span>해설 보기</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-[420px] w-full z-10 border border-slate-200 dark:border-slate-800 shadow-2xl">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border border-slate-200 dark:border-slate-850 bg-white dark:bg-slate-950 p-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 mb-5"
            />
            <div className="grid grid-cols-2 gap-3">
              <button
                onClick={() => setShowReportModal(false)}
                className="py-3 border border-slate-200 dark:border-slate-850 rounded-xl text-sm font-extrabold hover:bg-slate-50 dark:hover:bg-slate-850 text-slate-650 dark:text-slate-350"
              >
                취소
              </button>
              <button
                ...
                className="py-3 bg-blue-600 text-white rounded-xl text-sm font-extrabold hover:bg-blue-750 flex items-center justify-center gap-1.5 disabled:opacity-50"
              >
                {isReporting ? (
