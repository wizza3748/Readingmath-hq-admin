...
export default function MathExplanationPage(props: PageProps) {
  ...
  const handleBackToResult = () => {
    if (isPreview) {
      router.push(`/content/math-task-center/${taskId}/solve?preview=true`);
    } else {
      router.push(`/content/math-task-center/${taskId}/result${isPreview ? "?preview=true" : ""}`);
    }
  };
    ...
    <div className="flex h-screen flex-col bg-slate-50 text-slate-900 transition-colors duration-200 dark:bg-slate-950 dark:text-slate-100">
        ...
        >
          {(() => {
            ...
            return (
              ...
              >
                {/* 문항 헤더 */}
                <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-850 pb-4 flex-wrap gap-3">
                ...
                {/* 해설 영역 */}
                <div className="bg-blue-50/30 dark:bg-blue-950/10 border border-blue-100/50 dark:border-blue-900/30 rounded-2xl p-6 sm:p-7 space-y-3.5">
                  <h4 className="text-sm font-black text-blue-700 dark:text-blue-450 flex items-center gap-1.5">
                    <span className="flex h-5 w-5 items-center justify-center rounded-full bg-blue-100 dark:bg-blue-900 text-xs font-bold">i</span>
                    {idx + 1}번 해설
                  </h4>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-4 gap-3">
              {questions.map((q, idx) => {
                ...
                if (isActive) {
                  if (status === "correct") {
                    buttonClass += "border-2 border-blue-600 bg-blue-50 text-blue-900 dark:bg-blue-950/50 dark:text-blue-100 font-bold ring-2 ring-blue-500/20 scale-105 z-10";
                  ...
                  if (status === "correct") {
                    buttonClass += "border-transparent bg-blue-600 text-white font-semibold hover:bg-blue-700 hover:scale-105";
                  } else if (status === "incorrect") {
                    buttonClass += "border-transparent bg-rose-600 text-white font-semibold hover:bg-rose-700 hover:scale-105";
                  } else {
                    buttonClass += "border-gray-200 bg-white hover:bg-gray-50 text-gray-400 dark:border-gray-800 dark:bg-gray-900 dark:hover:bg-gray-850 dark:text-gray-500 hover:scale-105";
                  }
                }
      ...
      <footer className="fixed bottom-0 left-0 right-0 h-20 bg-white/95 dark:bg-slate-900/95 border-t border-slate-200 dark:border-slate-800 backdrop-blur-md z-45 flex items-center justify-between px-8 shadow-inner">
        ...
        <div className="flex items-center space-x-3">
          ...
          <button
            onClick={handleExit}
            className="px-6 py-3 bg-blue-600 hover:bg-blue-700 dark:bg-blue-650 dark:hover:bg-blue-550 text-white text-sm font-extrabold rounded-xl shadow-md transition-all active:scale-95 cursor-pointer"
          >
            과제 센터
          </button>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-slate-900 rounded-3xl p-6 max-w-[420px] w-full z-10 border border-slate-200 dark:border-slate-800 shadow-2xl">
            ...
            <textarea
              ...
              rows={4}
              className="w-full rounded-xl border border-slate-200 dark:border-slate-855 bg-white dark:bg-slate-950 p-3 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500 mb-5"
            />
