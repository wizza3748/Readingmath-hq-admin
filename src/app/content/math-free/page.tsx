...
// 수학용 코스 필터 (초3-1 ~ 중3-2까지만 데이터가 있으므로 14개만 캐러셀에 활용)
const MATH_COURSES = ALL_COURSES.slice(0, 14);

// 수학 캐릭터 매핑 (기본 모드 캐릭터 재사용)
...
// 수학 4대 계통 정의
const DOMAIN_TYPES = [
    { id: "num-ops", name: "수와 연산", icon: "🔢" },
    { id: "geom-measure", name: "도형과 측정", icon: "📐" },
    { id: "change-rel", name: "변화와 관계", icon: "📈" },
    { id: "data-prob", name: "자료와 가능성", icon: "📊" }
];

// 대단원명 기준으로 계통 자동 판정 함수
function getMathDomainOfUnit(majorUnit: string): string {
    const name = majorUnit.replace(/^\d+단원\s+/, "");
    // 수와 연산 관련 키워드
    if (/덧셈|뺄셈|곱셈|나눗셈|분수|소수|약수|배수|약분|통분|수의\s*범위|소인수|유리수|실수|제곱근|근호/i.test(name)) {
        return "수와 연산";
    }
    // 도형과 측정 관련 키워드
    if (/도형|삼각형|사각형|다각형|원|각도|길이|시간|들이|무게|둘레|넓이|부피|겉넓이|합동|대칭|입체|삼각비/i.test(name)) {
        return "도형과 측정";
    }
    // 자료와 가능성 관련 키워드
    if (/그래프|평균|통계|확률|자료/i.test(name)) {
        return "자료와 가능성";
    }
    // 그 외는 모두 변화와 관계 (문자와 식, 규칙, 대응, 방정식, 부등식, 함수, 비와 비율)
    return "변화와 관계";
}

// 계통별 테마 색상 설정
const DOMAIN_COLORS: Record<string, { bg: string; text: string; border: string }> = {
    "수와 연산": { bg: "bg-rose-500/10", text: "text-rose-400", border: "border-rose-500/20" },
    "도형과 측정": { bg: "bg-emerald-500/10", text: "text-emerald-400", border: "border-emerald-500/20" },
    "변화와 관계": { bg: "bg-indigo-500/10", text: "text-indigo-400", border: "border-indigo-500/20" },
    "자료와 가능성": { bg: "bg-amber-500/10", text: "text-amber-400", border: "border-amber-500/20" }
};

export default function MathFreePage() {
    ...
    const [selectedDomain, setSelectedDomain] = useState<string>("수와 연산");
    const [isPanelOpen, setIsPanelOpen] = useState(false);

    // Hydration 해결을 위해 마운트 후 localStorage 값 동기화
    useEffect(() => {
        setSelectedCourse(getGradeTerm("math") || "중1-1");
    ...
    // 캐러셀 관련 인덱스 연산
    const currentCourseIdx = MATH_COURSES.indexOf(selectedCourse);
    const handlePrevCourse = () => {
        const nextIdx = (currentCourseIdx - 1 + MATH_COURSES.length) % MATH_COURSES.length;
        setSelectedCourse(MATH_COURSES[nextIdx]);
        setIsPanelOpen(true);
    };
    const handleNextCourse = () => {
        const nextIdx = (currentCourseIdx + 1) % MATH_COURSES.length;
        setSelectedCourse(MATH_COURSES[nextIdx]);
        setIsPanelOpen(true);
    };
    ...
    const getUnitsForCourse = (courseCode: string) => {
        const courseData = MATH_CURRICULA.find(c => c.course === courseCode);
        ...
        courseData.types.forEach(t => {
            ...
            if (!majorUnitsMap[t.majorUnit].some(item => item.minorUnit === t.minorUnit)) {
                majorUnitsMap[t.majorUnit].push({
                    ...
                    domain: getMathDomainOfUnit(t.majorUnit)
                });
            }
        });
    ...
    const getUnitsForDomain = (domainName: string) => {
        ...
        MATH_CURRICULA.forEach(curr => {
            const courseUnits: Record<string, string[]> = {};
            curr.types.forEach(t => {
                const dom = getMathDomainOfUnit(t.majorUnit);
                if (dom === domainName) {
    ...
    return (
        <div className="relative min-h-screen w-full overflow-hidden bg-[#081324] font-sans select-none text-white">
                                    ...
                                    <div className="flex items-center gap-8 md:gap-16">
                                        ...
                                        >
                                            ...
                                            <h2 className="text-[24px] font-black tracking-tight mt-6 mb-2">
                                                {gradeTermToLabel(selectedCourse)}
                                            </h2>
                                        ...
                                        <div key={idx} className="space-y-3">
                                            ...
                                            <div className="space-y-2">
                                                {unitGroup.minors.map((minor, mIdx) => {
                                                    const color = DOMAIN_COLORS[minor.domain] || DOMAIN_COLORS["수와 연산"];
                                                    return (
                    ...
                    <div className="w-full pt-6">
                        {viewMode === "semester" ? (
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                {MATH_COURSES.map((courseCode, idx) => {
            ...
            <StudentSidebar
                ...
                subject="math"
                gradeTerm={gradeTermToLabel(selectedCourse)}
            />
        </div>
    );
}
