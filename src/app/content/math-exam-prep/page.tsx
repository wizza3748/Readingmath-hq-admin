...
import { getCombinedTypeHistory, evaluateAchievementStatus, getChallengeQuestionCount, MATH_TYPE_TO_QUESTIONS, getDailyAttemptsCount, isDailyAttemptAllowed, getOngoingSession } from "@/utils/examPrepStorage";
...
function AchievementIcon({ status, className }: { status: AchievementStatus; className?: string }) {
  ...
  if (cfg.icon === "x") return <span className={cn("inline-flex items-center justify-center font-black leading-none select-none", className)}>!</span>;
  if (cfg.icon === "alert") return <span className={cn("inline-flex items-center justify-center font-black leading-none select-none", className)}>!</span>;
...
function DetailPanel({ type, bigUnit, subUnit, onClose, isDark, gradeTerm }: DetailPanelProps) {
  ...
  // 진행중인 도전 세션 확인
  const ongoingSession = getOngoingSession("math", type.id);
  ...
  const combinedHistory = useMemo(() => {
    return getCombinedTypeHistory(type.id, "math");
  }, [type]);
        ...
        <div className="flex flex-col gap-2">
          {hasOngoing ? (
            <button
              onClick={() => {
                onClose();
                router.push(`/content/math-exam-prep/solve?typeId=${encodeURIComponent(type.id)}&name=${encodeURIComponent(type.name)}&gradeTerm=${encodeURIComponent(gradeTerm)}&sessionId=${ongoingSession.sessionId}`);
              }}
              className={cn(
...
function MathExamPrepPageContent() {
  ...
  const curriculum = useMemo<BigUnit[]>(() => {
    const rawCurriculum = MATH_CURRICULA.find(c => c.course === selectedGradeTerm);
    ...
    rawCurriculum.types.forEach((type, idx) => {
      ...
      if (!bigUnit) {
        bigUnitIndex++;
        const colors = ["bg-indigo-600", "bg-violet-600", "bg-sky-600", "bg-rose-600", "bg-amber-500", "bg-teal-600"];
        bigUnit = {
          id: `u${bigUnitIndex}`,
          badge,
          name: bigUnitName,
  ...
  const isFilterDefault =
    ...
    !onlyImportant &&
    selectedGradeTerm === "초3-1";

  const handleReset = () => {
    setSelectedTextbooks(new Set());
    setSelectedStatuses(new Set());
    setOnlyImportant(false);
    setSelectedGradeTerm("초3-1");
  ...
  // 테마 (과제 센터 스타일에 맞춤)
  const bg = isDark ? "bg-[#060413]" : "bg-[#f1f5f9]";
  const card = isDark ? "bg-[#1e293b] border-slate-700" : "bg-white border-slate-200";
  const filterBg = isDark ? "bg-[#0c0926]/90 border-white/[0.06]" : "bg-white/95 border-slate-200/80";
  const headerBg = isDark ? "bg-[#0c0926]/85 border-white/[0.06] shadow-[0_4px_30px_rgba(0,0,0,0.4)]" : "bg-white/95 border-slate-200/80 shadow-[0_2px_15px_rgba(0,0,0,0.04)]";
  const textPrimary = isDark ? "text-white" : "text-slate-800";
  const textMuted = isDark ? "text-slate-400" : "text-slate-500";
    ...
    <div className={cn("min-h-screen w-full font-sans", bg, isDark ? "text-slate-100" : "text-slate-900")}>
      ...
      <header className={`fixed top-0 left-0 right-0 h-[56px] ${headerBg} backdrop-blur-md border-b z-40 flex items-center justify-between px-6 transition-all duration-300`}>
          ...
          <div className="relative flex items-center justify-center h-8 w-8 rounded-lg bg-gradient-to-tr from-indigo-500 to-pink-500 p-[1.5px] transition-transform duration-300 group-hover:scale-105">
            <div className={`flex h-full w-full items-center justify-center rounded-[6px] ${isDark ? "bg-[#0c0926]" : "bg-white"}`}>
              <svg viewBox="0 0 100 100" className="h-4.5 w-4.5" xmlns="http://www.w3.org/2000/svg">
                <path d="M 50,10 L 55,45 L 90,50 L 55,55 L 50,90 L 45,55 L 10,50 L 45,45 Z" fill="url(#examPrepLogoGrad)" />
                <defs>
                  <linearGradient id="examPrepLogoGrad" x1="0%" y1="0%" x2="100%" y2="100%">
        ...
        <div className="flex items-end gap-1 h-full">
          ...
          <Link href="/content/math-task-center" className="h-[44px] flex items-center">
        ...
        <div className={`flex items-center gap-[20px] ${isDark ? "text-[#94a3b8]" : "text-slate-500"}`}>
          ...
          <Menu 
            className={`h-[20px] w-[20px] ${isDark ? "hover:text-white" : "hover:text-slate-900"} transition-colors cursor-pointer`} 
      ...
      <div className="w-full px-6 pt-[76px] pb-1 z-10">
        <div className="flex flex-wrap items-center gap-3.5 pb-2">
          ...
          <Link href="/content/science-exam-prep">
            <span className={`px-3 py-0.5 ${isDark ? 'bg-[#fbbf24]/10 text-[#fbbf24] border-[#fbbf24]/20 shadow-[0_0_15px_rgba(251,191,36,0.15)]' : 'bg-amber-50 text-amber-700 border-amber-200'} text-[12px] font-black rounded-full border cursor-pointer hover:opacity-80 transition-opacity`}>수학</span>
          </Link>
