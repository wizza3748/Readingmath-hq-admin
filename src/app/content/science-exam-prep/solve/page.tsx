...
import { SCIENCE_TYPE_TO_QUESTIONS, saveLocalPrepHistory, getCombinedTypeHistory, evaluateAchievementStatus, getChallengeQuestionCount, getDailyAttemptsCount, recordDailyAttempt, isDailyAttemptAllowed, getOngoingSession, saveOngoingSession, deleteOngoingSession } from "@/utils/examPrepStorage";
...
function ScienceSolveContent() {
  ...
  useEffect(() => {
    ...
    const existingOngoing = getOngoingSession("science", typeId);
    ...
    if (!isDailyAttemptAllowed(attemptParams)) {
      router.replace(`/content/science-exam-prep?selectedTypeId=${encodeURIComponent(typeId)}&gradeTerm=${encodeURIComponent(detectedGradeTerm)}`);
      return;
    }
  ...
  const handleSubmit = () => {
    ...
    if (isLastQuestion) {
      deleteOngoingSession("science", typeId);
    }
            ...
            {foundType.sampleQuestion && (
              <>
                ...
                <div className="flex flex-col gap-2.5">
                  <h4 className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest font-bold font-bold">대표 유형 문제</h4>
...
export default function ScienceSolvePage() {
  return (
    <Suspense fallback={
