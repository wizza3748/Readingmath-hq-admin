...
const getLevelComments = (level: number) => {
    const comments = {
        1: {
            title: "지금은 과학 개념들이 머릿속에서 각각 따로 정리되어 있어, 문제가 조금만 바뀌어도 어떻게 접근해야 할지 막막할 수 있고 문제를 읽을 때 중요한 조건이나 질문의 핵심을 놓쳐 아쉽게 틀리는 경우도 많으므로 단순히 정답만 외우는 공부보다는 “왜 이런 결과가 나왔을까?”를 스스로 질문하며 근거를 찾는 연습이 필요합니다. 차근차근 기초부터 개념을 연결해 나가면 실력은 충분히 성장할 수 있습니다.",
            roadmap: [
        ...
        2: {
            title: "“아는 문제인데 왜 틀렸지?”라는 질문을 자주 할 수 있습니다. 기본적인 내용은 이해하고 있지만, 문제가 조금만 복잡해지면 당황하기 쉽고, 몰라서 틀리기보다는 문제를 정확히 읽지 않거나, 개념연결 과정에서 어려움을 겪는 경우가 많습니다. 이제는 단순 암기보다는 배운 내용을 문제에 어떻게 적용할지, 그리고 왜 그 답이 정답인지 설명하는 연습에 집중할 필요가 있습니다.",
            roadmap: [
        ...
        3: {
            title: "“실수만 줄여도 점수가 눈에 띄게 오를 수 있는 단계”입니다. 기본 개념이나 대표적인 문제들은 안정적으로 해결하지만, 긴 지문이나 복잡한 자료가 제시될 때 미세한 조건을 놓치거나 계산 과정에서의 실수로 점수가 깎이곤 합니다. 이제는 정확하게 풀이하는 습관을 기르고, 고난도 응용 문제까지 차분하게 해결하는 연습이 필요합니다.",
            roadmap: [
        ...
        5: {
            title: "“보이지 않는 1점까지 놓치지 않는 실력 단계”입니다. 개념 이해, 문제 문해, 해결 과정, 추론 능력이 전반적으로 매우 안정적이며, 오답이 발생해도 대부분 단발성 실수이거나 고난도의 복합·비전형·정밀한 서술형 문제에서 나타납니다. 이 단계에서는 문제 수를 늘리기보다 정밀성을 유지하고, 고난도 문제로 사고의 폭을 확장하며, 논증 수준의 설명 능력을 더욱 고도화하는 것이 핵심입니다.",
            roadmap: [
