const courses = [
  ...
  { c: "초6-2", 지: ["분수의 나눗셈", "소수의 나눗셈", "공간과 입체", "비례식과 비례배분", "원의 넓이", "원기둥, 원뿔, 구"] },
...
const curricula = courses.map((courseObj, i) => {
  const units = courseObj.units || courseObj.지; // Fix typo
  const types = [];
  units.forEach((unitName, j) => {
    const majorUnit = `${j + 1}단원-${unitName}`;
    types.push({
      id: `mt-${courseObj.c}-${j}-1`,
      majorUnit,
      minorUnit: "기본 개념",
      typeName: `${unitName} 개념 이해`,
      difficultyCount: { basic: 10, intermediate: 5, advanced: 2 }
    });
    types.push({
      id: `mt-${courseObj.c}-${j}-2`,
      majorUnit,
      minorUnit: "응용 문제",
      typeName: `${unitName} 심화 적용`,
